...
def handler(event: dict, context) -> dict:
    ...
    if method == 'GET':
        ...
        if resource == 'projects':
            cur.execute(f'SELECT id, title, category, area, style, price, duration, image, gallery, sort_order FROM {t("site_projects")} ORDER BY sort_order')
            rows = cur.fetchall()
            conn.close()
        ...
        if resource == 'reviews':
            cur.execute(f'SELECT id, author, location, text, rating, sort_order FROM {t("site_reviews")} ORDER BY sort_order')
            rows = cur.fetchall()
            conn.close()
            cols = ['id','author','location','text','rating','sort_order']
            return {'statusCode': 200, 'headers': CORS_HEADERS, 'body': json.dumps([dict(zip(cols, r)) for r in rows])}

        cur.execute(f"SELECT section, key, value FROM {t('site_content')} WHERE section != '_sessions'")
        rows = cur.fetchall()
        conn.close()
        result = {}
    ...
    if action == 'save-content':
        ...
        for section, keys in sections.items():
            for key, value in keys.items():
                cur.execute(f'''
                    INSERT INTO {t("site_content")} (section, key, value) VALUES (%s, %s, %s)
    ...
    if action == 'save-project':
        ...
        cur = conn.cursor()
        if project_id:
            cur.execute(f'''UPDATE {t("site_projects")} SET title=%s, category=%s, area=%s, style=%s, price=%s,
            ...
            cur.execute(f'''INSERT INTO {t("site_projects")} (title, category, area, style, price, duration, image, gallery, sort_order)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, (SELECT COALESCE(MAX(sort_order),0)+1 FROM {t("site_projects")}))
                RETURNING id''',
                (body.get('title',''), body.get('category',''), body.get('area',''),
                 body.get('style',''), body.get('price',''), body.get('duration',''),
    ...
    if action == 'delete-project':
        conn = get_db()
        cur = conn.cursor()
        cur.execute(f'DELETE FROM {t("site_projects")} WHERE id = %s', (body.get('id'),))
        conn.commit()
        conn.close()
        return {'statusCode': 200, 'headers': CORS_HEADERS, 'body': json.dumps({'ok': True})}
    ...
    if action == 'save-review':
        ...
        cur = conn.cursor()
        if review_id:
            cur.execute(f'''UPDATE {t("site_reviews")} SET author=%s, location=%s, text=%s, rating=%s, sort_order=%s, updated_at=NOW() WHERE id=%s''',
                (body.get('author',''), body.get('location',''), body.get('text',''),
                 body.get('rating',5), body.get('sort_order',0), review_id))
        else:
            cur.execute(f'''INSERT INTO {t("site_reviews")} (author, location, text, rating, sort_order)
                VALUES (%s, %s, %s, %s, (SELECT COALESCE(MAX(sort_order),0)+1 FROM {t("site_reviews")}))''',
    ...
    if action == 'delete-review':
        conn = get_db()
        cur = conn.cursor()
        cur.execute(f'DELETE FROM {t("site_reviews")} WHERE id = %s', (body.get('id'),))
        conn.commit()
        conn.close()
        return {'statusCode': 200, 'headers': CORS_HEADERS, 'body': json.dumps({'ok': True})}
